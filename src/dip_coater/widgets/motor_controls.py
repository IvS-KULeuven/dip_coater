...
class MotorControls(Static):
    ...
    def _stop_and_disable_safely(self) -> Exception | None:
    ...
    def disable_after_abnormal_stop(self) -> None:
        ...
        error = self._stop_and_disable_safely()
        self.set_motor_state("fault" if error is not None else "disabled")
    ...
    async def disable_motor_action(self):
        ...
        if self.app_state.motor_state in ("moving", "homing"):
            ...
            if error is None:
                log.write(
                    "[dark_orange]Emergency stop: motor stopped and disabled.[/]"
                )
            else:
                log.write(
                    "[red]Emergency stop encountered a driver error after "
                    f"attempting stop and disable: {error}[/]"
                )
            ...
            try:
                ...
                self.set_motor_state("fault")
                log.write(f"[red]Motor disable failed: {error}[/]")
            else:
